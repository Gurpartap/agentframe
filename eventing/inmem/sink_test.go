...
func TestSink_EventsReturnsDeepClonedSnapshot(t *testing.T) {
	...
	messageEvent := agent.Event{
		RunID:   "run-1",
		Step:    1,
		Type:    agent.EventTypeAssistantMessage,
		Message: &message,
	}
	toolResultEvent := agent.Event{
		RunID:      "run-1",
		Step:       2,
		Type:       agent.EventTypeToolResult,
		ToolResult: &toolResult,
	}
	if err := sink.Publish(context.Background(), messageEvent); err != nil {
		t.Fatalf("publish message event: %v", err)
	}
	if err := sink.Publish(context.Background(), toolResultEvent); err != nil {
		t.Fatalf("publish event: %v", err)
	}

	messageEvent.Message.Content = "mutated"
	toolResultEvent.ToolResult.Content = "mutated"
	...
	if snapshot[1].ToolResult == nil || snapshot[1].ToolResult.Content != "result" {
		t.Fatalf("unexpected tool result snapshot: %+v", snapshot[1].ToolResult)
	}

	snapshot[0].Message.Content = "changed"
	snapshot[1].ToolResult.Content = "changed"

	next := sink.Events()
	if next[0].Message == nil || next[0].Message.Content != "hello" {
		t.Fatalf("snapshot mutation leaked into sink message: %+v", next[0].Message)
	}
	if next[1].ToolResult == nil || next[1].ToolResult.Content != "result" {
		t.Fatalf("snapshot mutation leaked into sink tool result: %+v", next[1].ToolResult)
	}
}
