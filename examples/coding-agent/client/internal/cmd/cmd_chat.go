...
func runChat(ctx context.Context, api *api.Client, baseURL string, input io.Reader, output io.Writer) error {
	...
	repl := chat.NewREPL(input, renderer, chat.Handlers{
...
func newChatController(ctx context.Context, api *api.Client, baseURL string, output io.Writer) *chatController {
	return &chatController{
		ctx:      ctx,
		api:      api,
		baseURL:  baseURL,
		state:    chat.NewState(),
		renderer: chat.NewRenderer(output, "chat> "),
	}
}
...
func (c *chatController) continueRun(ctx context.Context, maxSteps *int, resolution *chat.ResolutionInput) error {
	runID, _, ok := c.state.ActiveRun()
	if !ok {
		return errors.New("no active run; use /start first")
	}
	...
	if resolution == nil {
		...
		if current.Status == "suspended" {
			...
			return chat.NewResolutionRequiredError(chat.ResolutionPromptDefaults{
	...
	request := api.ContinueRequest{MaxSteps: maxSteps}
	if resolution != nil {
...
func (c *chatController) steer(ctx context.Context, instruction string) error {
	...
	state, _, err := c.api.Steer(ctx, runID, api.SteerRequest{
...
func (c *chatController) followUp(ctx context.Context, prompt string) error {
	...
	state, _, err := c.api.FollowUp(ctx, runID, api.FollowUpRequest{
		Prompt: prompt,
	})
	if err != nil {
		return err
...
func formatStreamEvent(frame events.StreamEvent) string {
	return fmt.Sprintf(
